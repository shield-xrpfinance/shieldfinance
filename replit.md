...
- **Testnet Launch Readiness (November 2025)**: Complete infrastructure hardening for public testnet launch including: (1) Direct WalletConnect transaction pattern refactored across all EVM flows (deposits, withdrawals, swaps, staking) - eliminates BrowserProvider hanging by using manual function encoding via `ethers.Interface` and direct `eth_sendTransaction` calls; (2) API rate limiting middleware (100 requests/min global, 20/min for sensitive endpoints) with in-memory store, automatic cleanup, and proper HTTP 429 responses; (3) RPC failover utility (shared/rpc-config.ts) with automatic endpoint testing, 5s timeout, and fallback for Coston2 (3 endpoints) and mainnet (3 endpoints); (4) Comprehensive testnet user guide (TESTNET_GUIDE.md, 460 lines) covering wallet setup, platform features, deposit/withdrawal flows, SHIELD staking with full unstake instructions, troubleshooting (rate limits, WalletConnect safety), and security best practices.