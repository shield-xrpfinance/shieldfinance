...
% Bitcoin-style minimal whitepaper with math support
% Compile with XeLaTeX or LuaLaTeX on Overleaf (or pdfLaTeX for fallback)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\usepackage{xcolor}
\usepackage{tikz}
\usetikzlibrary{shapes, arrows.meta, positioning, calc, decorations.pathreplacing, shadows, patterns}

% Font handling - works with pdfLaTeX, XeLaTeX, and LuaLaTeX
...
\ifxetexorluatex
  \usepackage{fontspec}
  % Try Inter font, fallback to Latin Modern
...
\hypersetup{
    colorlinks=true,
    linkcolor=shieldblue,
    urlcolor=shieldblue,
    citecolor=shieldblue
}
...
% Logo placeholder
\begin{tikzpicture}
    \draw[line width=3pt, shieldblue, fill=shieldlight, rounded corners=5pt] 
        (0,0) -- (3,0) -- (3,-2) -- (1.5,-3.5) -- (0,-2) -- cycle;
    \node[shieldblue, font=\Huge\bfseries] at (1.5,-1.3) {SHIELD};
    \draw[shieldgold, line width=2pt] (0.5,-0.3) -- (2.5,-0.3);
\end{tikzpicture}
...
\begin{tikzpicture}
    \draw[shieldblue, line width=1pt] (-4,0) -- (4,0);
\end{tikzpicture}
...
\begin{tabular}{rl}
    \textbf{Website} & \url{shyield.finance} \\[0.3em]
    \textbf{dApp} & \url{app.shyield.finance} \\[0.3em]
...
\node[box, below=1.5cm of vault] (shxrp) {shXRP Token};
\node[box, below=1.5cm of shxrp] (yield) {Yield Accrual};

% Arrows
\draw[arrow] (xrp) -- (xaman);
\draw[arrow] (xaman) -- (fassets);
\draw[arrow] (fassets) -- (fxrp);
\draw[arrow] (fxrp) -- (vault);
\draw[arrow] (vault) -- (shxrp);
\draw[arrow] (shxrp) -- (yield);

% Labels
\node[label, above=0.1cm of xrp] {User holds};
\node[label, above=0.1cm of xaman] {Sign tx};
\node[label, above=0.1cm of fassets] {Cross-chain};
\node[label, above=0.1cm of fxrp] {On Flare};
\node[label, above=0.1cm of vault] {ERC-4626};
\node[label, right=0.3cm of shxrp] {Liquid receipt};
\node[label, right=0.3cm of yield] {7--13\% + Boost};

% Withdraw path
\draw[arrow, dashed] (shxrp.west) -- ++(-2,0) |- node[label, pos=0.25, above] {Instant redeem} (xrp.south);
...
% Center node
\node[box, fill=shieldblue, text=white, minimum width=4cm] (revenue) {Protocol Revenue\\{\footnotesize 0.4\% fees}};

% Three branches
\node[box, fill=burnred!20, above left=1.5cm and 2cm of revenue] (burn) {50\% Burn\\{\footnotesize Buy \& burn SHIELD}};
\node[box, fill=boostgreen!20, above right=1.5cm and 2cm of revenue] (boost) {40\% Boost\\{\footnotesize FXRP to stakers}};
\node[box, fill=reservegray!20, below=1.5cm of revenue] (reserve) {10\% Reserves\\{\footnotesize Protocol treasury}};

% Arrows
\draw[arrow, burnred] (revenue) -- (burn);
\draw[arrow, boostgreen] (revenue) -- (boost);
\draw[arrow, reservegray] (revenue) -- (reserve);

% Flywheel effect
\node[box, right=2.5cm of boost] (stakers) {SHIELD Stakers\\{\footnotesize 30-day lock}};
\draw[arrow, boostgreen] (boost) -- (stakers);

\node[box, below=1cm of stakers] (shares) {Extra shXRP Shares\\{\footnotesize via donateOnBehalf}};
\draw[arrow, boostgreen] (stakers) -- (shares);

% Burn effect
\node[box, left=1.5cm of burn] (supply) {Reduced Supply\\{\footnotesize Deflationary}};
\draw[arrow, burnred] (burn) -- (supply);
...
\begin{tikzpicture}[scale=1.2]
    ...
    % Legend
    \node[right] at (4, 1.5) {\tikz\fill[burnred!70] (0,0) rectangle (0.4,0.4); \textbf{50\% Burn} -- Buy \& burn SHIELD (deflationary)};
    \node[right] at (4, 0.5) {\tikz\fill[boostgreen!70] (0,0) rectangle (0.4,0.4); \textbf{40\% Boost} -- FXRP to SHIELD stakers};
    \node[right] at (4, -0.5) {\tikz\fill[reservegray!70] (0,0) rectangle (0.4,0.4); \textbf{10\% Reserves} -- Protocol treasury};
...
\begin{center}
\begin{tikzpicture}
    \draw[shieldblue, line width=1pt] (-6,0) -- (6,0);
\end{tikzpicture}
...
\begin{center}
{\footnotesize All contracts verified on FlareScan. View source code at \url{github.com/shield-xrpfinance/shieldfinance}}
\end{center}
