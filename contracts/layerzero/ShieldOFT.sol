 ...
 * @notice LayerZero V2 OFT for SHIELD token on destination chains (Base, Arbitrum, Ethereum)
 * @dev Mints SHIELD when receiving from Flare, burns when sending back
 * 
 * This is NOT the canonical SHIELD token - it's a bridged representation.
 * The canonical token exists on Flare and is wrapped by ShieldOFTAdapter.
 * 
 * Security Features:
 ...
 * 
 * Token Properties:
 * - Same name/symbol as canonical SHIELD
 * - Only minter is the contract itself (via LayerZero receive)
 * - Supply is backed 1:1 by locked SHIELD on Flare
 */
contract ShieldOFT is ERC20, ERC20Burnable, Ownable, Pausable, ReentrancyGuard, IOFT {
    ...
    constructor(
        address _endpoint,
        address _owner
    ) ERC20("ShieldToken", "SHIELD") Ownable(_owner) {
        endpoint = ILayerZeroEndpointV2(_endpoint);
        decimalConversionRate = 10 ** (TOKEN_DECIMALS - SHARED_DECIMALS);
