  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] w-full text-foreground overflow-hidden relative" data-testid="page-security">
      ...
      <div className="relative z-10 container mx-auto px-4 py-12 max-w-5xl">
        ...
        >
          <Badge variant="outline" className="border-green-500/50 text-green-400 bg-green-900/10 px-4 py-1 text-sm uppercase tracking-wider mb-4" data-testid="badge-remediated">
          ...
          <div className="mt-12 pt-8">
            <div className="flex flex-col md:flex-row justify-center items-center gap-4 md:gap-0 relative">
              <div className="hidden md:block absolute top-1/2 left-10 right-10 h-0.5 bg-secondary -z-10 -translate-y-1/2" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-xl">Asfalia Security</span>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-col">
                    <span className="font-mono text-sm text-white">StakingBoost.sol</span>
                    <span className="font-mono text-sm text-white/70">ShieldToken.sol</span>
                  </div>
                </div>

                <div className="col-span-1 md:col-span-2 bg-secondary/30 rounded-lg p-4 border border-white/5">
                  <div className="flex justify-between items-center text-center gap-2">
                    <SeverityStat count={0} label="Critical" color="text-red-500" />
                    <div className="w-px h-8 bg-white/10" />
                    <SeverityStat count={1} label="High" color="text-orange-500" />
                    <div className="w-px h-8 bg-white/10" />
                    <SeverityStat count={4} label="Low" color="text-yellow-500" />
                    <div className="w-px h-8 bg-white/10" />
                    <SeverityStat count={7} label="Info" color="text-blue-400" />
                  </div>
                </div>
              </div>
            </CardContent>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16">
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between mb-6 flex-wrap gap-2">
              <h2 className="text-2xl font-bold">Findings & Remediations</h2>
            ...
            <Accordion type="single" collapsible defaultValue="item-1" className="space-y-4">
              ...
              <FindingItem 
                ...
                severityColor="bg-blue-500/10 text-blue-400 border-blue-500/20"
              ...
              <FindingItem 
                ...
                code="SB-04"
                severity="Medium"
                severityColor="bg-yellow-500/10 text-yellow-400 border-yellow-500/20"
              ...
              >
                <div className="p-3 bg-secondary/50 rounded border border-white/5 text-sm text-muted-foreground">
              ...
              <div className="p-4 bg-card/50 backdrop-blur rounded-lg border-dashed border border-white/10 flex items-center justify-center text-muted-foreground text-sm flex-wrap gap-2">
              ...
              <Card className="bg-card/50 backdrop-blur border-primary/20">
                ...
                <CardContent className="space-y-6">
                  ...
                  <a href="https://github.com/Shield-Finance/contracts" target="_blank" rel="noopener noreferrer">
                    <Button className="w-full bg-white/5 hover:bg-white/10 text-white border border-white/10" variant="outline" data-testid="button-github">
        ...
        >
          ...
          />
          <LinkCard 
            icon={<Bug className="w-6 h-6 text-green-400" />}
            title="Bug Bounty"
        ...
        <footer className="text-center border-t border-white/5 pt-12 pb-8">
          <p className="text-muted-foreground text-sm max-w-xl mx-auto">
            <AlertTriangle className="w-4 h-4 inline mr-2 text-yellow-500/50" />
  ...
  return (
    <div className="flex flex-col items-center relative z-10 md:w-1/4 mb-8 md:mb-0 group">
      <div className={`
        ...
        ${isCompleted ? "bg-green-900/20 border-green-500 text-green-500" : 
          isActive ? "bg-primary/20 border-primary text-primary scale-110" : 
      ...
      <h3 className={`font-bold text-sm mb-1 ${isActive ? "text-white" : "text-muted-foreground"}`}>{label}</h3>
...
function FindingItem({ id, code, severity, severityColor, title, description, children }: {
  ...
  return (
    <AccordionItem value={id} className="border border-white/5 bg-card/50 rounded-lg px-4 mb-2 overflow-hidden data-[state=open]:bg-card/80 transition-colors">
      ...
      <AccordionContent className="pb-4 pl-1 pt-0">
        <div className="ml-0 md:ml-[calc(100px+1rem)] space-y-4 border-l border-white/10 pl-4 mt-2">
...
function TestItem({ id, label }: { id: string, label: string }) {
  return (
    <li className="flex items-center gap-3 text-sm text-muted-foreground/80">
      <CheckCircle2 className="w-4 h-4 text-green-500/50 shrink-0" />
      <span className="font-mono text-xs text-white/30 mr-1">{id}</span>
      <span>{label}</span>
    </li>
  );
}
...
function LinkCard({ icon, title, description, action, href }: {
  ...
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" className="group block p-6 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 hover:border-primary/30 transition-all duration-300 hover:-translate-y-1" data-testid={`link-${title.toLowerCase().replace(/\s+/g, '-')}`}>
      <div className="mb-4 p-3 bg-black/20 w-fit rounded-lg border border-white/5 group-hover:scale-110 transition-transform">
        {icon}
      </div>
      <h3 className="font-bold text-lg text-white mb-2 group-hover:text-primary transition-colors">{title}</h3>
      <p className="text-muted-foreground text-sm mb-4">{description}</p>
      <div className="flex items-center text-sm font-medium text-white/50 group-hover:text-white">
